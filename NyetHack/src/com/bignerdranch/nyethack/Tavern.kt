...
import com.bignerdranch.nyethack.extensions.randomItem
import java.io.File
...
fun main() {
    ...
    (0..9).forEach{ _ ->
        val first = patronList.randomItem()
        val last = lastName.randomItem()
        val name = "$first $last"
        uniquePatrons += name
    }
    ...
    var orderCount = 0
    while(orderCount <= 9){
        placeOrder(uniquePatrons.randomItem(), menuList.randomItem())
        orderCount++
    }
